...
#define BlueLed 3  //--> Defining an BlueLed, used for indicators when the process of connecting to a wifi router
...
void setup() {
  ...
  digitalWrite(BlueLed, HIGH); //--> Turn off BlueLed On Board
  digitalWrite(YellowLed, HIGH); //--> Turn off YellowLed On Board
  digitalWrite(Buzzer, HIGH); //--> Turn off Buzzer On Board
...
void loop() {
  ...
  if (readsuccess) {
    ...
    http.begin(client, "http://192.168.1.6/clinic_ship/getUID.php");  //Specify request destination
